...
namespace TagManager.Tests
{
    public abstract class TagManagerTestBase : AbpIntegratedTestBase<TagManagerTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected TagManagerTestBase()
        {
        ...
        protected void UsingDbContext(Action<TagManagerDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<TagManagerDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<TagManagerDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<TagManagerDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<TagManagerDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<TagManagerDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<TagManagerDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<TagManagerDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<TagManagerDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<TagManagerDbContext, Task<T>> func)
        {
            T result;
